...
model.add(LSTM(128, batch_input_shape=(batch_size,500,56),stateful=True, return_sequences=True))
model.add(Activation('relu'))
model.add(Dropout(0.5))

model.add(LSTM(128, batch_input_shape=(batch_size,500,56),stateful=True))
model.add(Activation('relu'))
model.add(Dropout(0.5))
...
def batch_generator(pulses, batch_size):
    ...
    while True:
        ...
        if len(kb5[pulse]) < sample_size*30:
            continue
